...
import { CourseEntity, type CourseStatus } from '@/backend/course/domain/entities/course.entity';
import type { ICourseRepository } from '@/backend/course/domain/repositories/course.repository';
...
export class CourseService {
  ...
  async createCourse(dto: CreateCourseDto, creatorUid: string): Promise<CourseEntity> {
    try {
      const courseEntity = CourseEntity.create({
        ...
        referralPolicyId: dto.referralPolicyId === undefined ? null : dto.referralPolicyId,
      ...
      await catalogService.syncCatalogItem({
        assetReferenceId: courseEntity.id,
        assetType: 'course',
        creatorUid: courseEntity.creadorUid,
        publicName: courseEntity.nombre,
        coverUrl: courseEntity.imagenPortadaUrl,
        price: courseEntity.precio,
        currency: 'USD',
        status: courseEntity.estado === 'publicado' ? 'published' : (courseEntity.estado === 'archivado' ? 'archived' : 'draft'),
      });
  ...
  async updateCourse(courseId: string, dto: UpdateCourseDto, updaterUid: string): Promise<CourseEntity | null> {
    try {
      ...
      if (dto.referralPolicyId !== undefined) updateData.referralPolicyId = dto.referralPolicyId;
