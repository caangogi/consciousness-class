...
import type { IUserRepository, EnrolledCourseWithProgress, UserWithEnrolledCourses } from '@/backend/user/domain/repositories/user.repository';
import { UserEntity, type UserProperties } from '@/backend/user/domain/entities/user.entity';
import { adminDb } from '@/lib/firebase/admin';
import type { FirebaseError } from 'firebase-admin';
import { FieldValue, type Query } from 'firebase-admin/firestore';
import type { CourseProperties } from '@/backend/course/domain/entities/course.entity';
import type { UserCourseProgressProperties } from '@/backend/progress/domain/entities/user-course-progress.entity';
    ...
    } catch (error: any)
{
  ...
  async findUserWithEnrolledCoursesAndProgress(uid: string): Promise<UserWithEnrolledCourses | null> {
    ...
    try {
      ...
      return { ...userEntityPlain, enrolledCoursesDetails } as unknown as UserWithEnrolledCourses;
  ...
  async findAllUsers(limitCount: number = 20, orderByField: string = 'createdAt', orderDirection: 'asc' | 'desc' = 'desc'): Promise<UserEntity[]> {
    try {
      let query: Query = this.usersCollection;
      if (orderByField) {
